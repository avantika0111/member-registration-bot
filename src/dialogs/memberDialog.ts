...
export class MemberDialog extends ComponentDialog {
  private MemberInfo: StatePropertyAccessor<MemberInfo>;

  constructor(private userState: UserState) {
    super("memberDialog");

    this.MemberInfo = userState.createProperty(MEMBER_INFO);
    this.addDialog(new TextPrompt(INTRO_PROMPT));
  ...
  public async run(turnContext: TurnContext, accessor: StatePropertyAccessor) {
    const diaogSet = new DialogSet(accessor);
    diaogSet.add(this);
  ...
  private async roleStep(stepContext: WaterfallStepContext) {
    // WaterfallStep always finishes with the end of the Waterfall or with another dialog; here it is a Prompt Dialog.
    // Running a prompt here means the next WaterfallStep will be run when the users response is received.
    await stepContext.context.sendActivity(
      "Welcome to the Member Registration Bot!"
    );

    await stepContext.context.sendActivity(
      "I will ask you a few questions to get started. Let's start with your role."
    );
  ...
  private async nameStep(stepContext: WaterfallStepContext<MemberInfo>) {
  ...
  private async nameConfirmStep(stepContext: WaterfallStepContext<MemberInfo>) {
  ...
  private async confirmStep(stepContext: WaterfallStepContext<MemberInfo>) {
    stepContext.options.age = stepContext.result;

    const msg =
      stepContext.options.age === -1
        ? "No age given."
  ...
  private async summaryStep(stepContext: WaterfallStepContext<MemberInfo>) {
    if (stepContext.result) {
      // Get the current profile object from user state.
      const memberInfo = await this.MemberInfo.get(
        stepContext.context,
        new MemberInfo()
      );
  ...
  private async agePromptValidator(
    prompt: PromptValidatorContext<number>
  ): Promise<boolean> {
